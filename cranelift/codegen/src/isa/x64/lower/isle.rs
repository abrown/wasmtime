...
impl Context for IsleContext<'_, '_, MInst, X64Backend> {
    ...
    fn x64_xor_zero(&mut self, ty: Type) -> Gpr {
        use cranelift_assembler_x64::{inst, Gpr, GprMem};

        let write = self.temp_writable_gpr();
        let read = write.to_reg();
        let dst = Gpr::new(PairedGpr { read, write });
        let src = GprMem::Gpr(write.to_reg());
...
impl IsleContext<'_, '_, MInst, X64Backend> {
    ...
    /// Helper used by code generated by the `cranelift-assembler-x64` crate.
    fn convert_xmm_to_assembler_fixed_read_write_xmm<const E: u8>(
        &mut self,
        read: Xmm,
    ) -> asm::Fixed<PairedXmm, E> {
        let write = self.lower_ctx.alloc_tmp(types::F32X4).only_reg().unwrap();
        let write = WritableXmm::from_writable_reg(write).unwrap();
        asm::Fixed(PairedXmm { read, write })
    }
